...
var warn = AFRAME.utils.debug('components:look-at:warn');
var isCoordinates = coordinates.isCoordinates || coordinates.isCoordinate;

const lookAt = AFRAME.registerComponent('look-at', {
    schema: {
        default: '',
    ...
    update: function() {
        ...
        if (typeof target === 'object') {
            return object3D.lookAt(new Vector3(target.x, target.y, target.z));
        }
        ...
        return function(t) {
            ...
            if (target3D) {
                ...
                if (this.el.getObject3D('camera')) {
                    ...
                    // lookat from AFRAME.THREE camera objects, this is applied for you, but since the camera is
